...
private:
  template<typename... Args>
  friend class Luma::Core::Signal;
  friend class Luma::Core::Slots;
...
private:
  ...
  std::vector<Luma::Core::Signal<Args...>::ConnectionHandler> mConnectionHandlers;
};
...
std::shared_ptr<Connection> Signal<Args...>::connect (Handler &&handler)
{
  ...
  connectionSptr->setDisconnectFunc ([this] (const Connection *connection) {
    std::erase_if (mConnectionHandlers, [&] (const Luma::Core::Signal<Args...>::ConnectionHandler &connectonHandler) {
...
{
  for (Luma::Core::Signal<Args...>::ConnectionHandler &connectionHandler : mConnectionHandlers)
    {
