...
void Renderer::initialize () noexcept
{
  ...
  std::optional<GL::Shader> gShader = compiler.compileSource (
      "#version 150                                                                             \n"
      "                                                                                         \n"
      "layout (points) in;                                                                      \n"
  ...
  mShaderProgram = linker.link ({&vShader.value (), &gShader.value (), &fShader.value ()});
...
Renderer::Binder::Binder (const Renderer &renderer, const GL::ShaderProgramBinderInterface &binderInterface) noexcept : mRenderer (renderer), mBinderInterface (binderInterface)
{
}
...
Renderer::Binder &Renderer::Binder::draw (const Data &data) noexcept
{
  ...
  glVertexAttribIPointer (mRenderer.mAttributeCodeLoc, 1, GL_UNSIGNED_BYTE, 0, (void *) nullptr);

  glEnableVertexAttribArray (mRenderer.mAttributeCodeLoc);
  glDrawArrays (GL_POINTS /*mode*/, 0 /* first */, data.symbolsCount () /* count */);
  glDisableVertexAttribArray (mRenderer.mAttributeCodeLoc);
