...
void TextBoxShader::create () noexcept
{
  const char *vertexShaderSource = R"(#version 130
                                      ...
                                      void main ()
                                      {
                                        ...
                                        gl_Position.x += (gl_VertexID % uBoxWidth) * uSize.x;
                                        gl_Position.y -= (gl_VertexID / uBoxWidth) * uSize.y;
                                      }
                                    )";
  ...
  const char *fragmentShaderSource = R"(#version 130
                                     ...
                                     flat   in uint pAsciiCode;
                                     smooth in vec2 fGlyphUV;
  ...
  mUniformBoxWidthLoc                  = glGetUniformLocation (mProgramId, "uBoxWidth");
  ...
  if (isCreated ())
    {
      bind ();
      glUniform1i (mUniformBoxWidthLoc, mBoxWidth);
      glUniformMatrix4fv (mUniformModelViewProjectionMatrixLoc, 1, GL_FALSE, mModelViewProjectionMatrix);
      unbind ();
    }
}

bool TextBoxShader::isCreated () const noexcept
{
  return mProgramId;
}

void TextBoxShader::destroy () noexcept
{
  if (isCreated ())
    {
...
int TextBoxShader::textAttributeLocation () const
{
...
const float *TextBoxShader::modelViewProjectionMatrix () const noexcept
{
...
void TextBoxShader::setModelViewProjectionMatrix (const float *matrix) noexcept
{
